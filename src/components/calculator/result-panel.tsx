...
export function ResultPanel({ result }: { result: CalculationResult }) {
  ...
  const items = [
    ...
    ["9. Phí quảng cáo ước tính", formatVnd(result.adsFee)],
    ["11. Phí hoàn hàng ước tính", formatVnd(result.returnFee)],
    ["12. Phí vận hành ước tính", formatVnd(result.operationFee)],
    ["15. Giá bán sản phẩm", formatVnd(result.sellPrice)],
    ["16. Lãi thực tế", formatVnd(result.realProfit)],
    ["17. Lãi ròng", formatPercent(result.netMargin)],
    ["Điểm hòa vốn", formatVnd(result.breakEven)],
    ["ROAS Ads", result.roas > 0 ? `${result.roas.toFixed(2)}x` : "Không ads"],
  ];
  ...
  return (
    ...
    >
      <Card className="overflow-hidden">
        <CardHeader className="border-b bg-gradient-to-r from-primary to-sky-500 text-primary-foreground">
          <div className="flex items-center justify-between gap-3">
            <CardTitle className="text-primary-foreground">Kết quả</CardTitle>
        ...
        <CardContent className="space-y-4 pt-5">
          ...
          >
            ...
            {signals.warnings.length ? (
              ...
              <p className="text-sm">Biên lợi nhuận ổn định. ROAS mục tiêu gợi ý: {signals.suggestedRoas}x.</p>
            )}
            <div className="mt-3 flex items-center gap-2 text-sm">
              <TrendingUp className="h-4 w-4" />
              Safe CPC tham khảo: <strong>{formatVnd(result.safeCpc)}</strong>
            </div>
          </div>
        </CardContent>
      </Card>
